...
//     axios.post('http://localhost:3000/templates/create' , {
//       name:name ,
//       subject:subject,
...
const EmailCampaign = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000/templates").then((res) => {
      setTemplates(res.data);
    });

    axios.get("http://localhost:3000/emaillists").then((res) => {
  ...
  const handleSaveTemplateData = ({ name, subject, description }) => {
    axios
      .post("http://localhost:3000/templates/create", {
        name: name,
        subject: subject,
        description: description,
      })
